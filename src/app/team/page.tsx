...
export default function TeamPage() {
  ...
  const BatteryBlurb = "The Battery Team builds the high voltage lithium-ion battery pack that powers the car—it’s a subteam where mechanical and electrical work come together. With tasks ranging from pack design to modeling the container and cooling systems, we ensure the battery delivers power safely and reliably."
  ...
  const HVBlurb = "The High Voltage system ensures the energy inside the Tractive makes its way to the motor. This involves designing circuits that interface with the motor controller, placing high-current cabling to carry power, collaborating with other teams to optimally position tractive system components, and more! In addition to the on-board system, we also design and assemble a custom charger for the tractive battery. You might enjoy working on the HV system if you are interested in thermodynamics, power electronics, and cad."
